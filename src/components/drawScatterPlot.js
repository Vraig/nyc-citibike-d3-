...
export let drawScatterPlot = (scatterPlotLayer, data, xScale, yScale, tooltip, scatterPlotWidth, scatterPlotHeight) => {
    ...
    scatterPlotLayer.selectAll('.point') //select all the circle elements with the class 'point'
      ...
      .on("mouseover", (event, d) => {
        ...
        // Select all elements with the same class in both charts
      ...
      .on('mouseout',(event, d)=>{
        ...
        // Reset the color of all points with the same class
        d3.selectAll(`.point.${className}`)
          .style("fill", "steelblue")
          .lower();
          
        // Reset the color of all bars with the same class
